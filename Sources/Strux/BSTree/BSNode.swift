...
/// Node for a counted, self-balancing (AVL) binary search tree. Counted means that there are no duplicate nodes
/// with the same value, but values have counts associated with them. For a subtree to be a valid BST, the value
/// of the root node of the subtree must be **strictly** greater than any value in the sub-subtree, if any, with
/// its left child as root, and **strictly** less than any value in the sub-subtree, if any, with its right child
/// as root, and all sub-subtrees of the subtree must meet the same condition.
class BSNode<T: Comparable>: BNode {
    ...
    var parent: BNode {
        parentNode! // All BSNodes have parents, including root, but root's parent is not a BSNode.
    }
    ...
    // The node of having the next-lowest value, or nil if none
    ...
    /// The node having the next-highest value, or nil if none
